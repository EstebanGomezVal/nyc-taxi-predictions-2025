...
def register_champion_model(experiment_id: str) -> None:
    """Busca el run con el mejor RMSE y lo guarda como 
       "nyc-taxi-model-prefect" como el nuevo @champion"""
    
    name_model = "workspace.default.nyc-taxi-model-prefect"
    client = MlflowClient()

    best_run = mlflow.search_runs(experiment_ids=[experiment_id], 
                                  order_by=["metrics.rmse ASC"], max_results=1).iloc[0]
    best_run_id = best_run.run_id
    model_uri = f"runs:/{best_run_id}/model"

    try:
        client.get_registered_model(name=name_model)
    except:
        client.create_registered_model(name=name_model)

    model_version = client.create_model_version(source=model_uri, name=name_model, 
                                                run_id=best_run_id)

    client.set_registered_model_alias(name=name_model, alias="champion", 
...
def main_flow(year: int, month_train: str, month_val: str) -> None:
    ...
    # Register best model
    register_champion_model(experiment_id)
